...
from yolov11seg import YOLOv11SegONNX

def parse_args():
    parser = argparse.ArgumentParser(description="Run YOLOv11 Segmentation ONNX on image or webcam/video.")
    # Model / thresholds
    parser.add_argument("--model", type=str, default="./models/yolov11m-seg.onnx", help="Path to YOLOv11-Seg ONNX model.")
    parser.add_argument("--height", type=int, default=640, help="Model input height (e.g., 640).")
    parser.add_argument("--width", type=int, default=640, help="Model input width (e.g., 640).")
    ...
    parser.add_argument("--no-masks", action="store_true", help="Don't show segmentation masks (only bboxes).")
...
def run_image(args):
    """Run YOLOv11 Segmentation detection on a single image"""
    # Initialize detector
    yolo = YOLOv11SegONNX(
    ...
    t0 = time.time()
    detections, masks, result = yolo.detect_and_draw(image, show_masks=not args.no_masks)
    ...
    for i, det in enumerate(detections):
        ...
        mask_info = f" (mask: {np.sum(masks[i]):.0f} pixels)" if masks and i < len(masks) else ""
        print(f"  {i+1}. {class_name}: {conf:.3f} at [{x1:.0f}, {y1:.0f}, {x2:.0f}, {y2:.0f}]{mask_info}")
...
def run_webcam(args):
    """Run YOLOv11 Segmentation detection on webcam stream"""
    # Initialize detector
    yolo = YOLOv11SegONNX(
        model_path=args.model,
        conf_threshold=args.conf,
    ...
    print(f"Show masks: {not args.no_masks}")
    ...
    try:
        while True:
            ...
            # Inference
            t0 = time.time()
            detections, masks, vis = yolo.detect_and_draw(frame, show_masks=not args.no_masks)
            ...
            # Show window
            if not args.no_show:
                cv2.imshow("YOLOv11 Segmentation ONNX", vis)
                # Exit with q or ESC
...
def main():
    ...
    print("\nExamples:")
    print("  Image:   python run_yolov11seg.py --image test.jpg --output result.jpg")
    print("  Webcam:  python run_yolov11seg.py --webcam --gpu")
    print("  Save:    python run_yolov11seg.py --webcam --save-video output.mp4")
    print("  No mask: python run_yolov11seg.py --webcam --no-masks")
